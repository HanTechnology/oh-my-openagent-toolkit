...
4. **Refresh the session cache through the `/impeccable document` runtime.** Let the newly-written DESIGN.md land in conversation. Subsequent commands in this session will use the fresh version automatically without re-reading.
...
3. Refresh context through the `/impeccable document` runtime once so the seed lands in conversation for the rest of the session.
